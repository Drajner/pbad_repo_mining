...
We use sshtunel to get data from clickhouse server.
'''
class ClickhouseClient:
    def __init__(self,
        ...
        self.remote_user_passward = remote_user_passward
    ...
    '''
    execute the query and return the pandas dataframe
    '''
    def execute(self,sql):
        self.server.start()
        client = Client(host='127.0.0.1', port=self.server.local_bind_port, user=self.remote_user_name,
                        password=self.remote_user_passward,
                        database=self.database)
        rs = client.query_dataframe(sql)
        self.server.stop()
        return rs
    '''
