...
def visualize_activity_distribution():
    fontsize = 15
    df=pd.read_csv('data/pareto.csv',index_col=0)
    ...
    plt.tight_layout()
    plt.savefig(fname='imgs/pareto/all.png')
    plt.figure()
    ...
    plt.yticks(fontsize=fontsize)
    plt.tight_layout()
    plt.savefig(fname='imgs/pareto/cncf.png')
    plt.figure()
    ...
    plt.yticks(fontsize=fontsize)
    plt.tight_layout()
    plt.savefig(fname='imgs/pareto/projects.png')

'''
behavior distribution
'''
def visualize_behavior_distribution():
    df=pd.read_csv('data/behavior_distribution.csv',index_col=0)
    fontsize = 15
    df=df/10000
    ...
    plt.tight_layout()
    plt.savefig(fname='imgs/behavior_distribution/what-all.png')
    ...
    ax.legend(fontsize=fontsize)
    plt.tight_layout()
    plt.savefig(fname='imgs/behavior_distribution/what-cncf.png')
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig(fname='imgs/behavior_distribution/what-maturity.png')
    ...
    ax.legend(fontsize=fontsize)
    plt.tight_layout()
    plt.savefig(fname='imgs/behavior_distribution/what-project.png')
'''
working hour distribution
'''
def visualize_working_hour_distribution():
    def helper(name):
        fontsize = 15
        df = pd.read_csv('data/day_week_activity.csv',index_col=0)
        x = df['hour'].values
        y = df['week'].values
        ...
        plt.tight_layout()
        plt.savefig(fname=f'imgs/working_hour_distribution/{name}.png')
    for name in ['all','kubernetes','tikv']:
        helper(name)
'''
working hour distribution on their local time
'''
def visualize_working_hour_distribution_local_time():
    fontsize = 15
    df=pd.read_csv('data/day_week_activity_based_time_zone.csv',index_col=0)
    ...
    plt.legend(fontsize=fontsize)
    plt.tight_layout()
    plt.savefig(fname=f'imgs/working_hour_distribution_local_time/all.png')

'''
time zone distribution
'''
def visualize_time_zone_distribution():
    df=pd.read_csv('data/time_zone.csv',index_col=0)
    commit_df=pd.read_csv('data/commit_time_zone.csv',index_col=0)
    fontsize = 15
    for name,t1,t2 in [('cncf',16,15),
        ...
        fig.tight_layout()
        plt.savefig(fname=f'imgs/time_zone_distribution/{name}.png')
    ...
    plt.tight_layout()
    plt.savefig(fname=f'imgs/time_zone_distribution/all.png')



if __name__ == '__main__':
    visualize_activity_distribution()
    visualize_behavior_distribution()
    visualize_working_hour_distribution()
    visualize_working_hour_distribution_local_time()
    visualize_time_zone_distribution()